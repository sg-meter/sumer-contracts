...
/// @notice copied over from Solady
...
library FixedPointMathLib {
        ...
        sar(
          ...
          mul(
            add(
              ...
              sar(
                96,
                mul(add(24828157081833163892658089445524, sar(96, mul(add(3273285459638523848632254066296, x), x))), x)
              )
            ),
            x
          )
  ...
  function fullMulDivUnchecked(uint256 x, uint256 y, uint256 d) internal pure returns (uint256 z) {
    ...
    assembly {
      ...
      inv := mul(inv, sub(2, mul(d, inv)))
      inv := mul(inv, sub(2, mul(d, inv)))
      z := mul(or(mul(sub(p1, gt(r, z)), add(div(sub(0, t), t), 1)), div(sub(z, r), t)), mul(sub(2, mul(d, inv)), inv))
    }
  }
  ...
  function lerp(int256 a, int256 b, int256 t, int256 begin, int256 end) internal pure returns (int256) {
    ...
    // forgefmt: disable-next-item
    unchecked {
      if (b >= a) return int256(uint256(a) + fullMulDiv(uint256(b - a), uint256(t - begin), uint256(end - begin)));
